        ...
        <div class="bs-basket">
            ...
            <div class="row bs-basket--rel"  style="margin-bottom: 60px">
                <div class="col-sm-9 bs-basket-9">
                    <h5 class="bs-basket__heading"> <span><a href="{{ route('homepage') }}"><img src="/images/back.png" alt="back"></a></span> ВАША КОРЗИНА <span class="count">({{count(cart()->toArray()['items'])}})</span></h5>
                    
                    @foreach(cart()->toArray()['items'] as $product)
                    ...
                    <div class="row bs-basket__row">
                        ...
                        <div class="col-sm-3 bs-basket__qual">
                            ...
                            <div class="bs-basket__counter">
                                <form action="{{ route('cart.change.quantity') }}" method="POST" style="display: initial">
                                    {{ csrf_field() }}
                                    <input type="hidden" name="product" value="{{ $product['modelId'] }}">
                        ...
                        <div class="col-sm-3 bs-basket__cost">
                            <p class="bs-basket__cost">{{ number_format($product['price'] /  $product['quantity'], null, ' ', ' ') }}  тг / 1 шт.</p>
                            <h4 class="bs-basket__costTotal">{{ number_format($product['price'], null, ' ', ' ') }} тг / {{ $product['quantity'] }} шт.</h4>
                        </div>
                        <div class="col-sm-1 bs-basket__delete">
                            <a href="{{ route('removeToCart', ['id' => $product['modelId']]) }}"><img src="/images/delete.svg" alt="X" class=""></a>
                        </div>
                    </div>
                    <div class="row bs-basket__row-mob">
                        ...
                        <div class="col-xs-9 bs-basket__bigCol">
                          ...
                          <p class="bs-basket__cost">{{ number_format($product['price'] /  $product['quantity'], null, ' ', ' ') }}  тг</p>
                          <div class="bs-basket__counter">
                              <form action="{{ route('cart.change.quantity') }}" method="POST" style="display: initial">
                                  {{ csrf_field() }}
                                  <input type="hidden" name="product" value="{{ $product['modelId'] }}">
                ...
                <div class="row bs-basket__buttons">
                  <div class="row bs-basket__buttons-cost">
                      <p class="bs-basket__text bs-basket__text--total">ВЫБРАНО: <span class="count">{{count(cart()->toArray()['items'])}} товаров</span></p>
                      <p class="bs-basket__text bs-basket__text--total">ИТОГО: <span class="count">{{ number_format(10000, null, ' ', ' ') }} тг</span></p>
                  </div>
                ...
                <div class="col-sm-3 bs-basket-oform">
                    ...
                    <div>
                        ...
                        <p class="bs-basket__costText"> {{ number_format(10000, null, ' ', ' ') }} тг
                        </p>
                        <button class="bs-basket__order">
                            <a href="{{ route('checkout') }}"> Оформить заказ</a>
                        </button>
